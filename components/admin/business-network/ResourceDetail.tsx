...
function canCreateQuotationRevision(data: JsonRecord | null) {
  if (!data || typeof data.status !== "string") return false;
  return ["SENT", "VIEWED", "REJECTED"].includes(data.status);
}
  ...
  return <section className="space-y-5">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2">
        {config.editForm && data && permissions.has(config.editForm.permission) ? <EditBusinessResourceDialog endpoint={`${config.endpoint}/${encodeURIComponent(id)}`} label={config.editForm.label} fields={config.editForm.fields} record={data} onComplete={() => void load()} /> : null}
        {config.key === "quotations" && data && permissions.has("business.quotation.create") && canCreateQuotationRevision(data) ? <QuotationRevisionDialog endpoint={`${config.endpoint}/${encodeURIComponent(id)}`} record={data} onComplete={() => load()} /> : null}
        {config.actions?.filter((action) => permissions.has(action.permission) && isDetailActionAvailable(config.key, action, data, actorUserId)).map((action) => <Button key={action.slug} variant={action.tone === "danger" ? "destructive" : "outline"} onClick={() => openAction(action)}><ShieldCheck className="mr-2 h-4 w-4" />{action.label}</Button>)}
      </div>
    </div>
