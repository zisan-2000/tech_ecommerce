...
import { convertCustomerPurchaseOrderToOrder } from "@/lib/business-network/customer-po";
...
export async function POST(
  ...
  try {
    ...
    const actor = await requireBusinessNetworkAdminPermission("business.customer_po.convert");
    const { id } = await params;
    const customerPurchaseOrder = await convertCustomerPurchaseOrderToOrder({
