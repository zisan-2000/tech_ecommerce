...
import { approvePartnerAgreement } from "@/lib/business-network/partner";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    const actor = await requireBusinessNetworkAdminPermission("partner.agreement.approve");
    const { id } = await params;
    const agreement = await approvePartnerAgreement({
