...
import { partnerLeadReasonSchema } from "@/lib/business-network/partner-referral-schemas";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    const data = partnerLeadReasonSchema.parse(await readBusinessJsonBody(request));
    const lead = await updatePartnerLeadWorkflow({ id: (await params).id, action: "reject", reason: data.reason, actorUserId: actor.userId, request });
