...
import { duplicatePartnerLeadSchema } from "@/lib/business-network/partner-referral-schemas";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    const data = duplicatePartnerLeadSchema.parse(await readBusinessJsonBody(request));
    const lead = await updatePartnerLeadWorkflow({ id: (await params).id, action: "duplicate", duplicateOfId: data.duplicateOfId, actorUserId: actor.userId, request });
