...
import { sendSalesQuotation } from "@/lib/business-network/sales-quotation";
import { resourceIdSchema } from "@/lib/business-network/schemas";

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    assertSameOriginBusinessMutation(request);
    const actor = await requireBusinessNetworkAdminPermission("business.quotation.send");
    const { id } = await params;
    const quotation = await sendSalesQuotation({ id: resourceIdSchema.parse(id), actorUserId: actor.userId, request });
