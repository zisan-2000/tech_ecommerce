...
import { approveSalesQuotation } from "@/lib/business-network/sales-quotation";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const quotation = await approveSalesQuotation({ id: resourceIdSchema.parse(id), actorUserId: actor.userId, request });
