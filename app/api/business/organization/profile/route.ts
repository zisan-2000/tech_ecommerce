...
export async function GET() {
  try {
    const context = await requireBusinessPermission("organization.profile.read");
    return NextResponse.json(await getOrganizationPortalData(context, "profile"), { headers: { "Cache-Control": "private, no-store" } });
