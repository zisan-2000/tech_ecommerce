...
export async function GET() {
  try {
    const context = await requireBusinessPermission("organization.branches.read");
    return NextResponse.json(await getOrganizationPortalData(context, "branches"), { headers: { "Cache-Control": "private, no-store" } });
