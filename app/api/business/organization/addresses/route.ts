...
export async function GET() {
  try {
    const context = await requireBusinessPermission("organization.addresses.read");
    return NextResponse.json(await getOrganizationPortalData(context, "addresses"), { headers: { "Cache-Control": "private, no-store" } });
