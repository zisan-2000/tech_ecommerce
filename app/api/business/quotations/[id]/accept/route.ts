...
import { acceptSalesQuotation } from "@/lib/business-network/sales-quotation";
import { resourceIdSchema } from "@/lib/business-network/schemas";

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    assertSameOriginBusinessMutation(request);
    const context = await requireBusinessPermission("quotation.accept");
    await requireOrganizationCapability("CORPORATE_BUYER", context);
    const { id } = await params;
    const quotation = await acceptSalesQuotation({ context, id: resourceIdSchema.parse(id), request });
