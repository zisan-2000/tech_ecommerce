...
import { submitSalesRfq } from "@/lib/business-network/sales-rfq";
...
export async function POST(
  ...
  try {
    ...
    const context = await requireBusinessPermission("rfq.submit");
    await requireOrganizationCapability("CORPORATE_BUYER", context);
    const { id } = await params;
    const rfq = await submitSalesRfq({ context, id: resourceIdSchema.parse(id), request });
