  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx: any) => {
      ...
      for (const item of orderItemsData) if (item.product.type === "PHYSICAL") {
        ...
        else await deductVariantInventory({ tx, orderId: o.id, productId: item.product.id, productVariantId: item.variant.id, quantity: item.quantity, reason: `Order #${o.id} checkout deduction` });
      }
